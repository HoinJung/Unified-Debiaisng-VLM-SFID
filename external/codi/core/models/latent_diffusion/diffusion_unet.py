...
import sys
sys.path.append("/home/jw-server3/a/jung414/i-Code/i-Code-V3")
import torch
version = '0'
symbol = 'openai'
import copy
class image_Encoder(nn.Module):
    def __init__(self, input_dim=1280, feat_dim=32):
        super(image_Encoder, self).__init__()
        
        
        self.fc1 = nn.Linear(1280 , 512)
        self.fc2 = nn.Linear(512, 256)
        self.pool = nn.Conv2d(in_channels=1280, out_channels=1280, kernel_size=4, padding=0)
...
class image_Decoder(nn.Module):
    def __init__(self, input_dim=512, hidden_dim=256, output_channels=2, output_dim=1280):
        super(image_Decoder, self).__init__()
        self.fc1 = nn.Linear(input_dim, hidden_dim)
        self.fc2 = nn.Linear(hidden_dim, hidden_dim * 2*4*4)
        
        self.conv1 = nn.Conv2d(in_channels=512, out_channels=1280*2, kernel_size=(1, 1))
        self.leaky_relu = nn.LeakyReLU()
    ...
    def forward(self, z):
        ...
        recon = h.view(h.size(0), 2,1280,4,4)    
...
class UNetModelCoDi(nn.Module):
    ...
    def forward(self, x, timesteps, condition, xtype, condition_types, x_0=[None], x_0_type='first_frame', mix_weight={'audio': 1, 'text': 1, 'image': 1},\
                decoder_debias = False,gender=None,
        ...
        down_outcome = copy.deepcopy(h[0])
        # print(len(h))
        for i, xtype_i in enumerate(xtype):
            if xtype_i == 'audio':
                ...
                raise
        mid_outcome = copy.deepcopy(h[0])
        
            
        for cnt, (i_module, t_module, a_module, 
            i_con_in, t_con_in, a_con_in,) \
        in enumerate(zip(
            ...
            if cnt==0 and decoder_debias:
                ...
                # print(image_mean_features_lowconfidence_dec_down.shape)
                # print(temp[0].shape)
                values_to_assign = image_mean_features_lowconfidence_dec_down[image_important_indices_dec[0]].view(1, image_important_indices_dec[0].shape[0], 1, 1)
                temp[0][:,image_important_indices_dec[0],:,:] = values_to_assign
                ...
                h[0][:,image_important_indices_dec[1],:,:] = values_to_assign
        ...
        for i, xtype_i in enumerate(xtype):
            ...
            out_all.append(out)
        return out_all,(down_outcome,mid_outcome)
        # return out_all,mid_outcome

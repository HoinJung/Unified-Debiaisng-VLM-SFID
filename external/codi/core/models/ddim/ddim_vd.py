...
class DDIMSampler_VD(DDIMSampler):
    ...
    def sample(self,
        ...
        samples, intermediates,embeddings = self.ddim_sampling(
            shape,
            xt=xt,
            condition=condition,
        ...
        return samples, intermediates,embeddings
    ...
    def ddim_sampling(self, 
                      ...
                      temperature=1.,
                      mix_weight=None,
                      log_every_t=100,decoder_debias=None,
                      gender=None,
                      image_important_indices_dec = None,
               image_dec_results_dict = None):
        ...
        for i, step in enumerate(iterator):
            ...
            outs = self.p_sample_ddim(
                ...
                noise_dropout=noise_dropout,
                temperature=temperature,
                mix_weight=mix_weight,decoder_debias=decoder_debias,gender=gender,
                image_important_indices_dec = image_important_indices_dec,
               image_dec_results_dict = image_dec_results_dict)
            # pred_xt, pred_x0 = outs
            # print('check3')

            pred_xt, pred_x0,outcome = outs
            # print(outcome.shape)
            # embeddings.append(outcome)
            if index % log_every_t == 0 or index == total_steps - 1:
                intermediates['pred_xt'].append(pred_xt)
                intermediates['pred_x0'].append(pred_x0)

        # return pred_xt, intermediates,embeddings
        return pred_xt, intermediates,outcome
    ...
    def p_sample_ddim(self, x, 
                      ...
                      noise_dropout=0.,
                      temperature=1.,
                      mix_weight=None,decoder_debias=None,gender=None,
                      image_important_indices_dec = None,
               image_dec_results_dict = None):
        ...
        out,outcome = self.model.model.diffusion_model(
            x_in, t_in, condition, xtype=xtype, condition_types=condition_types, mix_weight=mix_weight,\
                decoder_debias=decoder_debias,gender=gender,image_important_indices_dec=image_important_indices_dec,
                image_dec_results_dict=image_dec_results_dict)
